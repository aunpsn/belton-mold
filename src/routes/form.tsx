...
function FormPage() {
  ...
  const { data: defectTypes = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("defect_types").select("*").order("name");
      if (error) throw error;
      return data ?? [];
    },
  });
    ...
    mutationFn: async () => {
      ...
      const payload = {
        ...
        defects: Object.fromEntries(
          Object.entries(defects).filter(([, v]) => v && Number(v) > 0)
                .map(([k, v]) => [k, Number(v)]),
        ),
        notes: notes || null,
      };
  ...
  async function exportXlsx() {
    ...
    const defectRows: (string | number)[][] = [
      [],
      ["Defects", "Count"],
      ...Object.entries(defects).filter(([, v]) => v).map(([k, v]) => [k, Number(v) || 0]),
    ];
    const ws = XLSX.utils.aoa_to_sheet([...header, ...rows, ...defectRows, [], ["Notes", notes]]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">ของเสีย (Defects)</CardTitle></CardHeader>
          ...
          {defectTypes.map((d) => (
            <Field key={d.id} label={d.name}>
              <Input
                ...
                inputMode="numeric"
                min={0}
                value={defects[d.name] ?? ""}
                onChange={(e) => setDefects({ ...defects, [d.name]: e.target.value })}
                placeholder="0"
              />
            </Field>
          ))}
